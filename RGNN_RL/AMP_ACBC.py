...
from ckt_graphs import GraphAMPACBC
...
CktGraph = GraphAMPACBC

class AMPACBCEnv(gym.Env, CktGraph, DeviceParams):
    ...
    def _do_simulation(self, action: np.array):
        ...
        try:
            ...
            AMP_ACBC_vars = open(f'{SPICE_NETLIST_DIR}/AMP_ACBC_vars.spice', 'r')
            lines = AMP_ACBC_vars.readlines()
            ...
            lines[7] = f'.param MOSFET_24_1_W_gm4_NMOS={W_M24} MOSFET_24_1_L_gm4_NMOS={L_M24} MOSFET_24_1_M_gm4_NMOS={M_M24}\n' 
            lines[8] = f'.param MOSFET_25_1_W_gm3_NMOS={W_M25} MOSFET_25_1_L_gm3_NMOS={L_M25} MOSFET_25_1_M_gm3_NMOS={M_M25}\n'
            lines[9] = f'.param MOSFET_59_1_W_ma1_PMOS={W_M59} MOSFET_59_1_L_ma1_PMOS={L_M59} MOSFET_59_1_M_ma1_PMOS={M_M59}\n'
            lines[10] = f'.param CURRENT_0_BIAS={Ib}\n'
            lines[11] = f'.param M_C0={M_C0}\n'
            lines[12] = f'.param M_C1={M_C1}\n'
            
            AMP_ACBC_vars = open(f'{SPICE_NETLIST_DIR}/AMP_ACBC_vars.spice', 'w')
            AMP_ACBC_vars.writelines(lines)
            AMP_ACBC_vars.close()
            os.system(f'cd {SPICE_NETLIST_DIR}&& ngspice -b -o AMP_ACBC_ACDC.log AMP_ACBC_ACDC.cir')
            os.system(f'cd {SPICE_NETLIST_DIR}&& ngspice -b -o AMP_ACBC_Tran.log AMP_ACBC_Tran.cir')
    ...
    def do_simulation(self, action):
        ...
        self.op_results = self.sim_results.dcop(file_name='AMP_ACBC_op')
    ...
    def _get_obs(self):
        ...
        try:
            f = open(f'{SPICE_NETLIST_DIR}/AMP_ACBC_op_mean_std.json')
        ...
        observation = np.array([
                               ...
                               [0,0,0,0,      0,      self.OP_M24_norm[0],self.OP_M24_norm[1],self.OP_M24_norm[2],self.OP_M24_norm[3],self.OP_M24_norm[4],self.OP_M24_norm[5],self.OP_M24_norm[6]],
    ...
    def _get_info(self):
        ...
        ''' DC '''
        self.dc_results = self.sim_results.dc(file_name='AMP_ACBC_ACDC_DC')
        ...
        ''' AC '''
        self.ac_results = self.sim_results.ac(file_name='AMP_ACBC_ACDC_AC')
        ...
        if self.dcgain > 0 :
            ...
            self.GBW_PM_results = self.sim_results.GBW_PM(file_name='AMP_ACBC_ACDC_GBW_PM')
        ...
        """ Tran """
        self.tran_results = self.sim_results.tran(file_name='AMP_ACBC_Tran')
        ...
        self.time_data, self.vin_data, self.vout_data = self.sim_results.extract_tran_data(file_name='AMP_ACBC_tran.dat')
        ...
        self.Active_Area_1 = (self.W_M0 * self.L_M0 * self.M_M0 * 9 + self.W_M9 * self.L_M9 * self.M_M9 * 2 + self.W_M11 * self.L_M11 * self.M_M11 * 1 +\
                           self.W_M12 * self.L_M12 * self.M_M12 * 1 + self.W_M17 * self.L_M17 * self.M_M17 * 2 + self.W_M18 * self.L_M18 * self.M_M18 * 7 +\
                           self.W_M22 * self.L_M22 * self.M_M22 * 2 + self.W_M24 * self.L_M24 * self.M_M24 * 1 + self.W_M25 * self.L_M25 * self.M_M25 * 1 + self.W_M59 * self.L_M59 * self.M_M59 * 1  ) * 1e-6 + \
    ...
    def _init_random_sim(self, max_sims=100):
        ...
        while random_op_count <= max_sims :
            ...
            op_results = sim_results.dcop(file_name='AMP_ACBC_op')
        ...
        with open(f'{SPICE_NETLIST_DIR}/AMP_ACBC_op_mean_std.json','w') as file:
