...
from ckt_graphs import GraphAMPDFCFC1
...
CktGraph = GraphAMPDFCFC1

class AMPDFCFC1Env(gym.Env, CktGraph, DeviceParams):
    ...
    def _do_simulation(self, action: np.array):
        ...
        try:
            ...
            AMP_DFCFC1_vars = open(f'{SPICE_NETLIST_DIR}/AMP_DFCFC1_vars.spice', 'r')
            lines = AMP_DFCFC1_vars.readlines()
            ...
            AMP_DFCFC1_vars = open(f'{SPICE_NETLIST_DIR}/AMP_DFCFC1_vars.spice', 'w')
            AMP_DFCFC1_vars.writelines(lines)
            AMP_DFCFC1_vars.close()
            os.system(f'cd {SPICE_NETLIST_DIR}&& ngspice -b -o AMP_DFCFC1_ACDC.log AMP_DFCFC1_ACDC.cir')
            os.system(f'cd {SPICE_NETLIST_DIR}&& ngspice -b -o AMP_DFCFC1_Tran.log AMP_DFCFC1_Tran.cir')
    ...
    def do_simulation(self, action):
        ...
        self.op_results = self.sim_results.dcop(file_name='AMP_DFCFC1_op')
    ...
    def _get_obs(self):
        ...
        try:
            f = open(f'{SPICE_NETLIST_DIR}/AMP_DFCFC1_op_mean_std.json')
    ...
    def _get_info(self):
        ...
        ''' DC '''
        self.dc_results = self.sim_results.dc(file_name='AMP_DFCFC1_ACDC_DC')
        ...
        ''' AC '''
        self.ac_results = self.sim_results.ac(file_name='AMP_DFCFC1_ACDC_AC')
        ...
        if self.dcgain > 0 :
            ...
            self.GBW_PM_results = self.sim_results.GBW_PM(file_name='AMP_DFCFC1_ACDC_GBW_PM')
        ...
        """ Tran """
        self.tran_results = self.sim_results.tran(file_name='AMP_DFCFC1_Tran')
        ...
        self.time_data, self.vin_data, self.vout_data = self.sim_results.extract_tran_data(file_name='AMP_DFCFC1_tran.dat')
        ...
        self.Active_Area_1 = (self.W_M0 * self.L_M0 * self.M_M0 * 9 + self.W_M9 * self.L_M9 * self.M_M9 * 2 + self.W_M11 * self.L_M11 * self.M_M11 * 1 +\
                           ...
                           self.W_M22 * self.L_M22 * self.M_M22 * 2 + self.W_M24 * self.L_M24 * self.M_M24 * 1 + self.W_M25 * self.L_M25 * self.M_M25 * 1 ) * 1e-6 + \
    ...
    def _init_random_sim(self, max_sims=100):
        ...
        while random_op_count <= max_sims :
            ...
            op_results = sim_results.dcop(file_name='AMP_DFCFC1_op')
        ...
        with open(f'{SPICE_NETLIST_DIR}/AMP_DFCFC1_op_mean_std.json','w') as file:
