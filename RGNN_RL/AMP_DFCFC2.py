...
from ckt_graphs import GraphAMPDFCFC2
...
CktGraph = GraphAMPDFCFC2

class AMPDFCFC2Env(gym.Env, CktGraph, DeviceParams):
    ...
    def _do_simulation(self, action: np.array):
        ...
        try:
            ...
            AMP_DFCFC2_vars = open(f'{SPICE_NETLIST_DIR}/AMP_DFCFC2_vars.spice', 'r')
            lines = AMP_DFCFC2_vars.readlines()
            ...
            lines[2] = f'.param MOSFET_10_1_W_gm4_PMOS={W_M10} MOSFET_10_1_L_gm4_PMOS={L_M10} MOSFET_10_1_M_gm4_PMOS={M_M10}\n'
            lines[3] = f'.param MOSFET_11_1_W_gm2_PMOS={W_M11} MOSFET_11_1_L_gm2_PMOS={L_M11} MOSFET_11_1_M_gm2_PMOS={M_M11}\n'
            lines[4] = f'.param MOSFET_12_1_W_gmf2_PMOS={W_M12} MOSFET_12_1_L_gmf2_PMOS={L_M12} MOSFET_12_1_M_gmf2_PMOS={M_M12}\n'
            lines[5] = f'.param MOSFET_18_7_W_BIASCM_NMOS={W_M18} MOSFET_18_7_L_BIASCM_NMOS={L_M18} MOSFET_18_7_M_BIASCM_NMOS={M_M18}\n'
            lines[6] = f'.param MOSFET_23_2_W_LOAD2_NMOS={W_M23} MOSFET_23_2_L_LOAD2_NMOS={L_M23} MOSFET_23_2_M_LOAD2_NMOS={M_M23}\n'
            lines[7] = f'.param MOSFET_25_1_W_gm3_NMOS={W_M25} MOSFET_25_1_L_gm3_NMOS={L_M25} MOSFET_25_1_M_gm3_NMOS={M_M25}\n'
            lines[8] = f'.param CURRENT_0_BIAS={Ib}\n'
            lines[9] = f'.param M_C0={M_C0}\n'
            lines[10] = f'.param M_C1={M_C1}\n'
            
            AMP_DFCFC2_vars = open(f'{SPICE_NETLIST_DIR}/AMP_DFCFC2_vars.spice', 'w')
            AMP_DFCFC2_vars.writelines(lines)
            AMP_DFCFC2_vars.close()
            os.system(f'cd {SPICE_NETLIST_DIR}&& ngspice -b -o AMP_DFCFC2_ACDC.log AMP_DFCFC2_ACDC.cir')
            os.system(f'cd {SPICE_NETLIST_DIR}&& ngspice -b -o AMP_DFCFC2_Tran.log AMP_DFCFC2_Tran.cir')    
    ...
    def do_simulation(self, action):
        ...
        self.op_results = self.sim_results.dcop(file_name='AMP_DFCFC2_op')
    ...
    def _get_obs(self):
        ...
        try:
            f = open(f'{SPICE_NETLIST_DIR}/AMP_DFCFC2_op_mean_std.json')
    ...
    def _get_info(self):
        ...
        ''' DC '''
        self.dc_results = self.sim_results.dc(file_name='AMP_DFCFC2_ACDC_DC')
        ...
        ''' AC '''
        self.ac_results = self.sim_results.ac(file_name='AMP_DFCFC2_ACDC_AC')
        ...
        if self.dcgain > 0 :
            ...
            self.GBW_PM_results = self.sim_results.GBW_PM(file_name='AMP_DFCFC2_ACDC_GBW_PM')
        ...
        """ Tran """
        self.tran_results = self.sim_results.tran(file_name='AMP_DFCFC2_Tran')
        ...
        self.time_data, self.vin_data, self.vout_data = self.sim_results.extract_tran_data(file_name='AMP_DFCFC2_tran.dat')
        ...
        self.Active_Area_1 = (self.W_M0 * self.L_M0 * self.M_M0 * 8 + self.W_M8 * self.L_M8 * self.M_M8 * 2 + self.W_M10 * self.L_M10 * self.M_M10 * 1 +\
                           self.W_M11 * self.L_M11 * self.M_M11 * 1 + self.W_M12 * self.L_M12 * self.M_M12 * 1 +  self.W_M18 * self.L_M18 * self.M_M18 * 10 +\
                           self.W_M23 * self.L_M23 * self.M_M23 * 2 + self.W_M25 * self.L_M25 * self.M_M25 * 1 ) * 1e-6 + \
    ...
    def _init_random_sim(self, max_sims=100):
        ...
        while random_op_count <= max_sims :
            ...
            op_results = sim_results.dcop(file_name='AMP_DFCFC2_op')
        ...
        with open(f'{SPICE_NETLIST_DIR}/AMP_DFCFC2_op_mean_std.json','w') as file:
