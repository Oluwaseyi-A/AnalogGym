...
from ckt_graphs import GraphAMPNMCNR
...
CktGraph = GraphAMPNMCNR

class AMPNMCNREnv(gym.Env, CktGraph, DeviceParams):
    ...
    def _do_simulation(self, action: np.array):
        ...
        try:
            ...
            AMP_NMCNR_vars = open(f'{SPICE_NETLIST_DIR}/AMP_NMCNR_vars.spice', 'r')
            lines = AMP_NMCNR_vars.readlines()
            ...
            lines[3] = f'.param MOSFET_17_7_W_BIASCM_NMOS={W_M17} MOSFET_17_7_L_BIASCM_NMOS={L_M17} MOSFET_17_7_M_BIASCM_NMOS={M_M17}\n'
            lines[4] = f'.param MOSFET_21_2_W_LOAD2_NMOS={W_M21} MOSFET_21_2_L_LOAD2_NMOS={L_M21} MOSFET_21_2_M_LOAD2_NMOS={M_M21}\n'
            ...
            lines[8] = f'.param M_C1={M_C1}\n'
            lines[9] = f'.param M_R0={M_R0}\n'
            
            AMP_NMCNR_vars = open(f'{SPICE_NETLIST_DIR}/AMP_NMCNR_vars.spice', 'w')
            AMP_NMCNR_vars.writelines(lines)
            AMP_NMCNR_vars.close()

            os.system(f'cd {SPICE_NETLIST_DIR}&& ngspice -b -o AMP_NMCNR_ACDC.log AMP_NMCNR_ACDC.cir')
            os.system(f'cd {SPICE_NETLIST_DIR}&& ngspice -b -o AMP_NMCNR_Tran.log AMP_NMCNR_Tran.cir')
    ...
    def do_simulation(self, action):
        ...
        self.op_results = self.sim_results.dcop(file_name='AMP_NMCNR_op')
    ...
    def _get_obs(self):
        ...
        try:
            f = open(f'{SPICE_NETLIST_DIR}/AMP_NMCNR_op_mean_std.json')
        ...
        observation = np.array([
                               ...
                               [0,0,0,0,0,       0,      self.OP_M21_norm[0],self.OP_M21_norm[1],self.OP_M21_norm[2],self.OP_M21_norm[3],self.OP_M21_norm[4],self.OP_M21_norm[5],self.OP_M21_norm[6]],
                               [0,0,0,0,0,       0,      self.OP_M22_norm[0],self.OP_M22_norm[1],self.OP_M22_norm[2],self.OP_M22_norm[3],self.OP_M22_norm[4],self.OP_M22_norm[5],self.OP_M22_norm[6]],
    ...
    def _get_info(self):
        ...
        ''' DC '''
        self.dc_results = self.sim_results.dc(file_name='AMP_NMCNR_ACDC_DC')
        ...
        ''' AC '''
        self.ac_results = self.sim_results.ac(file_name='AMP_NMCNR_ACDC_AC')
        ...
        if self.dcgain > 0 :
            ...
            self.GBW_PM_results = self.sim_results.GBW_PM(file_name='AMP_NMCNR_ACDC_GBW_PM')
        ...
        """ Tran """
        self.tran_results = self.sim_results.tran(file_name='AMP_NMCNR_Tran')
        ...
        self.time_data, self.vin_data, self.vout_data = self.sim_results.extract_tran_data(file_name='AMP_NMCNR_tran.dat')
        ...
        self.R0 = self.M_R0 * 0.4767
        self.Active_Area_1 = (self.W_M0 * self.L_M0 * self.M_M0 * 9 + self.W_M8 * self.L_M8 * self.M_M8 * 2 + self.W_M10 * self.L_M10 * self.M_M10 * 1 +\
                           self.W_M17 * self.L_M17 * self.M_M17 * 9 + \
    ...
    def _init_random_sim(self, max_sims=100):
        ...
        while random_op_count <= max_sims :
            ...
            op_results = sim_results.dcop(file_name='AMP_NMCNR_op')
        ...
        with open(f'{SPICE_NETLIST_DIR}/AMP_NMCNR_op_mean_std.json','w') as file:
