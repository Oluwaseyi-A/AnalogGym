...
from ckt_graphs import GraphAMPPFC
...
CktGraph = GraphAMPPFC

class AMPPFCEnv(gym.Env, CktGraph, DeviceParams):
    ...
    def _do_simulation(self, action: np.array):
        ...
        try:
            ...
            AMP_PFC_vars = open(f'{SPICE_NETLIST_DIR}/AMP_PFC_vars.spice', 'r')
            lines = AMP_PFC_vars.readlines()
            ...
            lines[3] = f'.param MOSFET_11_1_W_gmf1_PMOS={W_M11} MOSFET_11_1_L_gmf1_PMOS={L_M11} MOSFET_11_1_M_gmf1_PMOS={M_M11}\n'
            ...
            AMP_PFC_vars = open(f'{SPICE_NETLIST_DIR}/AMP_PFC_vars.spice', 'w')
            AMP_PFC_vars.writelines(lines)
            AMP_PFC_vars.close()
            os.system(f'cd {SPICE_NETLIST_DIR}&& ngspice -b -o AMP_PFC_ACDC.log AMP_PFC_ACDC.cir')
            os.system(f'cd {SPICE_NETLIST_DIR}&& ngspice -b -o AMP_PFC_Tran.log AMP_PFC_Tran.cir')            
    ...
    def do_simulation(self, action):
        ...
        self.op_results = self.sim_results.dcop(file_name='AMP_PFC_op')
    ...
    def _get_obs(self):
        ...
        try:
            f = open(f'{SPICE_NETLIST_DIR}/AMP_PFC_op_mean_std.json')
    ...
    def _get_info(self):
        ...
        ''' DC '''
        self.dc_results = self.sim_results.dc(file_name='AMP_PFC_ACDC_DC')
        ...
        ''' AC '''
        self.ac_results = self.sim_results.ac(file_name='AMP_PFC_ACDC_AC')
        ...
        if self.dcgain > 0 :
            ...
            self.GBW_PM_results = self.sim_results.GBW_PM(file_name='AMP_PFC_ACDC_GBW_PM')
        ...
        """ Tran """
        self.tran_results = self.sim_results.tran(file_name='AMP_PFC_Tran')
        ...
        self.time_data, self.vin_data, self.vout_data = self.sim_results.extract_tran_data(file_name='AMP_PFC_tran.dat')
    ...
    def _init_random_sim(self, max_sims=100):
        ...
        while random_op_count <= max_sims :
            ...
            op_results = sim_results.dcop(file_name='AMP_PFC_op')
        ...
        with open(f'{SPICE_NETLIST_DIR}/AMP_PFC_op_mean_std.json','w') as file:
