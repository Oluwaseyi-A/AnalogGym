...
from ckt_graphs import GraphAMPRAFFC
...
CktGraph = GraphAMPRAFFC

class AMPRAFFCEnv(gym.Env, CktGraph, DeviceParams):
    ...
    def _do_simulation(self, action: np.array):
        ...
        try:
            ...
            AMP_RAFFC_vars = open(f'{SPICE_NETLIST_DIR}/AMP_RAFFC_vars.spice', 'r')
            lines = AMP_RAFFC_vars.readlines()
            lines[0] = f'.param MOSFET_0_8_W_BIASCM_PMOS={W_M0} MOSFET_0_8_L_BIASCM_PMOS={L_M0} MOSFET_0_8_M_BIASCM_PMOS={M_M0}\n'
            lines[1] = f'.param MOSFET_7_1_W_gm3_PMOS={W_M7} MOSFET_7_1_L_gm3_PMOS={L_M7} MOSFET_7_1_M_gm3_PMOS={M_M7}\n'
            ...
            lines[4] = f'.param MOSFET_11_1_W_gmf_PMOS={W_M11} MOSFET_11_1_L_gmf_PMOS={L_M11} MOSFET_11_1_M_gmf_PMOS={M_M11}\n'
            lines[5] = f'.param MOSFET_15_2_W_gmb_NMOS={W_M15} MOSFET_15_2_L_gmb_NMOS={L_M15} MOSFET_15_2_M_gmb_NMOS={M_M15}\n'
            ...
            AMP_RAFFC_vars = open(f'{SPICE_NETLIST_DIR}/AMP_RAFFC_vars.spice', 'w')
            AMP_RAFFC_vars.writelines(lines)
            AMP_RAFFC_vars.close()
            os.system(f'cd {SPICE_NETLIST_DIR}&& ngspice -b -o AMP_RAFFC_ACDC.log AMP_RAFFC_ACDC.cir')
            os.system(f'cd {SPICE_NETLIST_DIR}&& ngspice -b -o AMP_RAFFC_Tran.log AMP_RAFFC_Tran.cir')
    ...
    def do_simulation(self, action):
        ...
        self.op_results = self.sim_results.dcop(file_name='AMP_RAFFC_op')
    ...
    def _get_obs(self):
        ...
        try:
            f = open(f'{SPICE_NETLIST_DIR}/AMP_RAFFC_op_mean_std.json')
    ...
    def _get_info(self):
        ...
        ''' DC '''
        self.dc_results = self.sim_results.dc(file_name='AMP_RAFFC_ACDC_DC')
        ...
        ''' AC '''
        self.ac_results = self.sim_results.ac(file_name='AMP_RAFFC_ACDC_AC')
        ...
        if self.dcgain > 0 :
            ...
            self.GBW_PM_results = self.sim_results.GBW_PM(file_name='AMP_RAFFC_ACDC_GBW_PM')
        ...
        """ Tran """
        self.tran_results = self.sim_results.tran(file_name='AMP_RAFFC_Tran')
        ...
        self.time_data, self.vin_data, self.vout_data = self.sim_results.extract_tran_data(file_name='AMP_RAFFC_tran.dat')
        ...
        self.Active_Area_1 = (self.W_M0 * self.L_M0 * self.M_M0 * 7 + self.W_M7 * self.L_M7 * self.M_M7 * 1 + self.W_M8 * self.L_M8 * self.M_M8 * 2 + self.W_M10 * self.L_M10 * self.M_M10 * 1 +\
                           self.W_M11 * self.L_M11 * self.M_M11 * 1 + self.W_M15 * self.L_M15 * self.M_M15 * 2 + \
                           self.W_M17 * self.L_M17 * self.M_M17 * 8 + self.W_M21 * self.L_M21 * self.M_M21 * 2   ) * 1e-6 + \
    ...
    def _init_random_sim(self, max_sims=100):
        ...
        while random_op_count <= max_sims :
            ...
            op_results = sim_results.dcop(file_name='AMP_RAFFC_op')
        ...
        with open(f'{SPICE_NETLIST_DIR}/AMP_RAFFC_op_mean_std.json','w') as file:
