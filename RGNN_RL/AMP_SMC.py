...
from ckt_graphs import GraphAMPSMC
...
CktGraph = GraphAMPSMC

class AMPSMCEnv(gym.Env, CktGraph, DeviceParams):
    ...
    def _do_simulation(self, action: np.array):
        ...
        try:
            ...
            AMP_SMC_vars = open(f'{SPICE_NETLIST_DIR}/AMP_SMC_vars.spice', 'r')
            lines = AMP_SMC_vars.readlines()
            ...
            AMP_SMC_vars = open(f'{SPICE_NETLIST_DIR}/AMP_SMC_vars.spice', 'w')
            AMP_SMC_vars.writelines(lines)
            AMP_SMC_vars.close()
            os.system(f'cd {SPICE_NETLIST_DIR}&& ngspice -b -o AMP_SMC_ACDC.log AMP_SMC_ACDC.cir')
            os.system(f'cd {SPICE_NETLIST_DIR}&& ngspice -b -o AMP_SMC_Tran.log AMP_SMC_Tran.cir')
    ...
    def do_simulation(self, action):
        ...
        self.op_results = self.sim_results.dcop(file_name='AMP_SMC_op')
    ...
    def _get_obs(self):
        ...
        try:
            f = open(f'{SPICE_NETLIST_DIR}/AMP_SMC_op_mean_std.json')
    ...
    def _get_info(self):
        ...
        ''' DC '''
        self.dc_results = self.sim_results.dc(file_name='AMP_SMC_ACDC_DC')
        ...
        ''' AC '''
        self.ac_results = self.sim_results.ac(file_name='AMP_SMC_ACDC_AC')
        ...
        if self.dcgain > 0 :
            ...
            self.GBW_PM_results = self.sim_results.GBW_PM(file_name='AMP_SMC_ACDC_GBW_PM')
        ...
        """ Tran """
        self.tran_results = self.sim_results.tran(file_name='AMP_SMC_Tran')
        ...
        self.time_data, self.vin_data, self.vout_data = self.sim_results.extract_tran_data(file_name='AMP_SMC_tran.dat')
        ...
        if self.settlingTime > 0 and self.Active_Area >0 and self.TC > 0 and self.vos > 0 :
            ...
            self.FOM_AMP = ( (self.PSRR/self.PSRR_base) * (self.cmrrdc/self.cmrrdc_base) * (self.dcgain/self.dcgain_base) * (self.FOMS/self.FOMS_base) * \
                           (self.FOML/self.FOML_base) ) * ((self.settlingTime/self.settlingTime_base) * (self.Active_Area/self.Active_Area_base)) ** (-1) * \
                           ((self.TC/self.TC_base) **(-1)) * indicator_TC * ((self.vos/self.vos_base) **(-1))* indicator_vos  
        else :
            self.FOM_AMP = 0
    ...
    def _init_random_sim(self, max_sims=100):
        ...
        while random_op_count <= max_sims :
            ...
            op_results = sim_results.dcop(file_name='AMP_SMC_op')
        ...
        with open(f'{SPICE_NETLIST_DIR}/AMP_SMC_op_mean_std.json','w') as file:
